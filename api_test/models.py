...
class GlobalHost(models.Model):
    ...
    description = models.CharField(max_length=1024, blank=True, null=True, verbose_name='描述')
    key = models.CharField(max_length=50, blank=True, null=True, verbose_name='头部key')
    value = models.CharField(max_length=5000, blank=True, null=True, verbose_name='头部value')
    username=models.CharField(max_length=50, blank=True, null=True, verbose_name='获取token的账号')
    password = models.CharField(max_length=50, blank=True, null=True, verbose_name='获取token的密码')
    url = models.CharField(max_length=500, blank=True, null=True, verbose_name='登录地址')
    status = models.BooleanField(default=True, verbose_name='状态')
    LastUpdateTime = models.DateTimeField(blank=True, null=True,auto_now=True, verbose_name='最近修改时间')
...
class GlobalToken(models.Model):
    ...
    id = models.AutoField(primary_key=True)
    project = models.ForeignKey(Project, on_delete=models.CASCADE, verbose_name='项目')
    key = models.CharField(max_length=50, blank=True, null=True, verbose_name='值')
    value = models.CharField(max_length=5000, blank=True, null=True, verbose_name='值')
    username=models.CharField(max_length=50, blank=True, null=True, verbose_name='获取token的账号')
    password = models.CharField(max_length=50, blank=True, null=True, verbose_name='获取token的密码')
    url = models.CharField(max_length=500, blank=True, null=True, verbose_name='登录地址')
    status = models.BooleanField(default=True, verbose_name='状态')
    GlobalHost = models.ForeignKey(GlobalHost, on_delete=models.CASCADE, verbose_name='host')
    LastUpdateTime = models.DateTimeField(blank=True, null=True,auto_now=True, verbose_name='最近修改时间')
    createTime = models.DateTimeField(blank=True, null=True,auto_now_add=True, verbose_name='创建时间')
...
class DataBase(models.Model):
    ...
    CHOICES = (('1', 'Mysql'),
        ('2', 'SQL Server'),)
    type = models.CharField(max_length=50, verbose_name='数据库类型', choices=CHOICES)
    ...
    name = models.CharField(max_length=50, verbose_name='自定义名称')
    class Meta:
        verbose_name = '数据库信息'
...
class ApiHead(models.Model):
    ...
    value = models.CharField(max_length=5000, blank=True, null=True, verbose_name='内容')
...
class AutomationCaseApi(models.Model):
    ...
    exeSequence=models.FloatField(max_length=20, blank=True, null=True,verbose_name='执行顺序')
...
class ParameterVariable(models.Model):
    """
     公共变量___获取请求参数的值
     """
    id = models.AutoField(primary_key=True)
    automationTestCase = models.ForeignKey(AutomationTestCase, on_delete=models.CASCADE,
                                                   verbose_name='用例')
    automationCaseApi = models.ForeignKey(AutomationCaseApi,
                                          on_delete=models.CASCADE, verbose_name='接口')
    name = models.TextField(max_length=50,blank=True, null=True, verbose_name='变量名字')
    value = models.TextField(max_length=50, blank=True, null=True, verbose_name='变量名字')
    custom = models.TextField(max_length=50, blank=True, null=True, verbose_name='自定义')

class PublicVariable(models.Model):
    """
     公共变量————自定义的（随机数、身份证号）
     """
    id = models.AutoField(primary_key=True)
    name = models.TextField(max_length=100,blank=True, null=True, verbose_name='变量名字')
    value = models.TextField(max_length=100, blank=True, null=True, verbose_name='变量名字')
    custom = models.TextField(max_length=100, blank=True, null=True, verbose_name='自定义')
...
class AutomationHead(models.Model):
    ...
    automationCaseApi = models.ForeignKey(AutomationCaseApi, related_name='header',
                                          on_delete=models.CASCADE, verbose_name='接口')
    name = models.CharField(max_length=1024, verbose_name='参数名')
    value = models.CharField(max_length=5000, verbose_name='内容')
    interrelate = models.BooleanField(default=False, verbose_name='是否响应关联')
...
class AutomationParameter(models.Model):
    ...
    interrelate = models.BooleanField(default=False, verbose_name='是否响应关联')
    parInterrelate = models.BooleanField(default=False, verbose_name='是否参数关联')
    _type = models.CharField(default="String", max_length=50, verbose_name='参数类型', blank=True, null=True,
                             choices=(('Int', 'Int'), ('String', 'String'), ('Bool', 'Bool'), ('List', 'List'), ('Dict', 'Dict')))
...
class AutomationResponsParameter(models.Model):
    """
    存储校验返回值key-value
    """
    id = models.AutoField(primary_key=True)
    automationCaseApi = models.ForeignKey(AutomationCaseApi, related_name='ResponsParameterList',
                                                 on_delete=models.CASCADE, verbose_name='接口')
...
class AutomationParameterRaw(models.Model):     # TODO 待调整关联关系：automationCaseApi
    """
    自动化模块：请求的源数据参数--raw
    """
    id = models.AutoField(primary_key=True)
    automationCaseApi = models.OneToOneField(AutomationCaseApi, related_name='parameterRaw',  # origin
                                             on_delete=models.CASCADE, verbose_name='接口')
    # automationCaseApi = models.OneToOneField(ApiParameterRaw, related_name='data',
    #                                          on_delete=models.CASCADE, verbose_name='接口')
...
class AutomationTestResult(models.Model):
    ...
    header = models.CharField(max_length=5000, blank=True, null=True, verbose_name='请求头')
...
class AutomationTestTask(models.Model):
    ...
    frequency = models.CharField(max_length=50,blank=True, null=True, verbose_name='参数')
...
class AutomationCaseTestResult(models.Model):
    ...
    """
    id = models.AutoField(primary_key=True)
    automationCaseApi = models.ForeignKey(AutomationCaseApi, on_delete=models.CASCADE, verbose_name='接口', related_name="auto_result")
    header = models.CharField(max_length=5000, blank=True, null=True, verbose_name='请求头')
    parameter = models.TextField(blank=True, null=True, verbose_name='请求参数')
    result = models.CharField(max_length=50, verbose_name='测试结果', choices=RESULT_CHOICE)
    httpStatus = models.CharField(max_length=50, blank=True, null=True, verbose_name='http状态', choices=HTTP_CODE_CHOICE)
    ...
    responseData = models.TextField(blank=True, null=True, verbose_name='实际返回内容')
    caseUrl = models.TextField(blank=True, null=True, verbose_name='实际请求地址')
