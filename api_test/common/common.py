...
def check_json(src_data, dst_data):     # TODO 需重新调整json校验算法
    """
    校验json
    :param src_data:  校验内容
...
def record_auto_results(_id, time,  header, parameter, _result, responseHeader, code, response_data,caseUrl):
    """
    记录自动测试结果
    :param _id: ID
    :param time:  测试时间
    ...
    :return:
    """
    result_ = AutomationCaseTestResult(automationCaseApi=AutomationCaseApi.objects.get(id=_id), header=header,caseUrl=caseUrl,
                                       parameter=parameter, testTime=time, responseHeader=responseHeader,
                                       result=_result, httpStatus=code, responseData=response_data)
    result_.save()


def create_json(api_id, api_name, data):
    """
    根据json数据生成关联数据接口
    :param api_id: 接口ID
    :param data: Json数据
    ...
    if isinstance(data, dict):
        ...
        # for i in data:
        #     m = (api+"[\"%s\"]" % i)
        AutomationResponseJson(automationCaseApi=api_id, name=api_name, tier=data, type='json').save()
