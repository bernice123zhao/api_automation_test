    ...
    <section>
            ...
            <el-table-column label="操作" min-width="20%">
                <template slot-scope="scope">
                    ...
                    <el-button type="danger" size="small" @click="handleDel(scope.$index, scope.row)">删除</el-button>
                        <el-button type="success" size="small" @click="CopyApi(scope.$index, scope.row)">复制</el-button>
                </template>
            </el-table-column>
        </el-table>
        <el-col :span="24" class="toolbar">
        ...
        <el-dialog title="测试结果" :visible.sync="TestResult" :close-on-click-modal="false">
            <div style="height:700px;overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6">
                ...
                <div class="resultStyle">测试结果规则内容</div>
                <div v-show="result.data" class="resultStyle" style="overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6;padding: 10px;width: 90%;word-break: break-all;line-height:25px"><pre v-highlightjs><code v-text=result.data></code></pre></div>
...
<!--                <div class="resultStyle" style="overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6;padding: 10px;width: 90%;word-break: break-all;line-height:25px"><code >{{result.responseData}}</code></div>-->
<!--                 <div class="resultStyle">{{result.responseData}}</div>-->
                <div class="resultStyle" style="overflow:auto;overflow-x:hidden;border: 1px solid #e6e6e6;padding: 10px;width: 90%;word-break: break-all;line-height:25px">
                    <pre v-highlightjs><code v-text=result.responseData></code></pre></div>
    ...
    export default {
        ...
        methods: {
            ...
            getDefalutHost(){
                ...
                console.log(Object.keys(this.Host));
                 console.log(Object.values(this.Host));
                console.log(this.Host[0]["name"]);
                this.url=this.Host[0]["id"]
            },
            ...
            CopyApi(index, row){
                    ...
                    $.ajax({
                        type: "post",
                        url: test+"/api/automation/CopyApi",
                        async: true,
                        data: JSON.stringify({
                           ids: row.id ,
                      }),
                    ...
                    success: (data) => {
                        if (data.code === '999999') {
                            ...
                            // self.result["parameter"] = data.data.parameter;
                              self.result["parameter"] = eval('(' + data.data.parameter + ')');
                            ...
                            self.result["responseData"] = eval('(' + data.data.responseData + ')');
            ...
            handleCurrentChangeApi(val) {
                this.page = val;
                console.log(this.activeIndex)
                console.log(this.groupData[this.activeIndex])
                this.getApiList(this.groupData[this.activeIndex])
