<template>
    <section>
        ...
        <el-button class="return-list" type="primary" style="float: right; margin-right: 15px" @click.native="addApi">
            保存
        </el-button>
        <el-form :model="form" ref="form" :rules="FormRules">
            ...
            <el-row :span="24">
                <el-collapse v-model="activeNames" @change="handleChange">
                    ...
                    <el-dialog title="响应关联用例" :visible.sync="searchApiVisible" :close-on-click-modal="false">
                        <el-row :gutter="10">
                            <el-col :span="6">
                        ...
                        <el-card class="box-card">
                            <div slot="header" class="clearfix">
                                <el-radio-group v-model="form.check">
                                    ...
                                    <el-radio-button label="calibrated">
                                        <div>关联校验</div>
                                    </el-radio-button>
                                </el-radio-group>
                            ...
                            <div v-show="showCheck">
                                ...
                                <!--   关联校验参数化-->
                                <el-table v-if="form.check==='calibrated'" :data="form.calibrated_para"
                                    ...
                                    <el-table-column min-width="7%">
                                        <template slot-scope="scope">
                                            <el-button type="primary" size="mini" style="margin-bottom: 5px"
                                                       v-if="scope.row.interrelate"
                                                       @click="handleCorrelation(scope.$index, scope.row)">关联1
                                            </el-button>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="操作" min-width="7%">
                                        <template slot-scope="scope">
                                            <i class="el-icon-delete" style="font-size:30px;cursor:pointer;"
                                               @click="delCalibrated(scope.$index)"></i>
                                        </template>
                                    </el-table-column>
                                    <el-table-column label="" min-width="5%">
                                        <template slot-scope="scope">
                                            <el-button v-if="scope.$index===(form.calibrated_para.length-1)" size="mini"
                                                       class="el-icon-plus" @click="addCalibrated"></el-button>
                                        </template>
                                    </el-table-column>
                                </el-table>
                                <el-input v-if="form.check!=='calibrated'" style="margin-top: 10px"
                                          v-model.trim="form.checkData" type="textarea"
...
<script>
    import {test, get_parameter, get_api_list} from '../../../api/api'
    import $ from 'jquery'
    ...
    export default {
        data() {
         ...
         paramTyep: [ {value: 'String', label: 'String'},{value: 'Int', label: 'Int'},
                ...
                form: {
                    ...
                    Http4: 'HTTP',
                    addr: '',
                    head: [{name: "Content-Type", value: "application/json; charset=utf-8", interrelate: 0,},
                    ...
                    checkData: "",
                    _type: "",
                    calibrated_para: [{name: "", value: "", interrelate: 0,},//校验关联
                        {name: "", value: "", interrelate: 0}],
                },
                FormRules: {
        ...
        methods: {
            ...
            addInterrelateSubmit() {
                ...
                try {
                    if (this.LinkCategory === 'URL') {
                        ...
                        this.interrelateObjects['parInterrelate'] = false;
                    }
                } catch (e) {
            ...
            addCalibrated() {
                let headers = {name: "", value: "", interrelate: 0,};
                this.form.calibrated_para.push(headers)
            },
            delCalibrated(index) {
                this.form.calibrated_para.splice(index, 1);
                if (this.form.calibrated_para.length === 0) {
                    this.form.calibrated_para.push({name: "", value: "", interrelate: 0,})
                }
            },
            changeParameterType() {
